...
function setvar()
	...
	for i=1,math.min(lines,5) do 
		...
		if info[index] ~= nil and info[index] ~= "PLACEHOLDER" then
			...
			SKIN:Bang('!SetOption', 'Delete'..(i),'LeftMouseUpAction','[play "#@#"][!ShowMeterGroup Dialogue][!SetVariable ToDelete '..(i+position-1)..'][!SetVariable ToDeleteName "'..text..'"][!Update]')
...
function Invoke(number)
	if number ~= nil then
		...
		SKIN:Bang('!SetOption','EditOKButton','LeftMouseUpAction','[!HideMeterGroup SetIconAll][!Update][!Delay 300][!CommandMeasure Dock Edit('..number..')]')
		if formatl == "3" and sort ~= nil and ascending ~= nil then 
			...
			SKIN:Bang('!SetOption','EditPathTitle','Y','6R')
		...
		elseif format == "1" then
			...
			SKIN:Bang('!SetOption','EditPathTitle','Y','174')
		...
		SKIN:Bang('!SetOption','EditBackground','H','202')
		SKIN:Bang('!SetOption','EditOKButton','LeftMouseUpAction','[!HideMeterGroup SetIconAll][!Update][!Delay 300][!CommandMeasure Dock Edit()]')
	end

function Initialize()
	...
	builtindata={
		...
		'<Command>g</Command><Action>https://www.google.com/search?q=#Action#</Action>',
		'<Command>info</Command><Action>[!ActivateConfig NXT-OS\\info info.ini]</Action>',
		'<Command>help</Command><Action>[!ActivateConfig "NXT-OS\\System\\Help" "Help.ini"][!CommandMeasure "Script" """GoTo("Scout")""" "NXT-OS\\System\\Help"]</Action>',
...
function Command()
	...
	local actioninput = string.match(input, "%s([a-zA-Z0-9%(%)%.%:%/%_ ]+)")
	...
	elseif commandinput == "day" or commandinput == "date" or commandinput == "today" or commandinput == "now" then
		...
		SKIN:Bang("!SetOption","Result","Text",os.date("Today is %A, %B %d"..postfix..", %Y."))
		SKIN:Bang('[!Update][!CommandMeasure "Animate" "Execute 3"]')
		ToggleResult(true)
			...
			for k, v in pairs(info) do
				...
				if string.lower(commandinput) == string.lower(command) then
					...
					if string.match(execute,"[%[]*[%]]") then
						SKIN:Bang(execute)
						SKIN:Bang('[!SetOption "Icon" "ImageTint" "#Color.Main#"][!SetOption "Background" "ImageTint" "#Color.Main#,150"][!Update][!CommandMeasure "Script" "ToggleResult(false)"]')
					else
						SKIN:Bang('"'..execute..'"')
						SKIN:Bang('[!SetOption "Icon" "ImageTint" "#Color.Main#"][!SetOption "Background" "ImageTint" "#Color.Main#,150"][!Update][!CommandMeasure "Script" "ToggleResult(false)"]')
					end

...
function gentables()
	for k,v in pairs(uncompleted) do uncompleted[k]=nil end
	for k,v in pairs(completed) do completed[k]=nil end
	local counter = 1
	for k,v in ipairs(all) do
		local text, date, done, remind = string.match(v[2], "<Text>(.-)</Text><Date>(.-)</Date><Done>(.-)</Done><Remind>(.-)</Remind>")
		if done == "0" then
			uncompleted[counter]={k,v[2]}
			counter = counter+1
		end
	end
	local counter = 1
...
function write()
	...
	SKIN:Bang('!Refresh','NXT-OS\\Overlay\\Widgets\\Reminders')
end

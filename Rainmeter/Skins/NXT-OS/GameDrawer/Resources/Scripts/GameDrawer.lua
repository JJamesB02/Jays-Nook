function Initialize()
	...
	if SELF:GetOption("Refreshed", "0") == "0" then
		...
		sortMode = SKIN:GetVariable('GameDrawer.SortMode') --The current sort mode that Game Drawer is using.
		if sortMode == nil then 
			sortMode = "LastPlayed"
		elseif sortMode == "Alphabetically" then
...
function ToggleSort()
	if sortMode == "Alphabetically" then
		sortMode = "LastPlayed"
		SKIN:Bang('[!SetOption SortButton ImageName Resources\\Images\\SortByTime.png][!WriteKeyValue Variables GameDrawer.SortMode LastPlayed "#@#Settings.inc"]')
	elseif sortMode == "LastPlayed" then
		sortMode = "Alphabetically"
		SKIN:Bang('[!SetOption SortButton ImageName Resources\\Images\\Alphabetically.png][!WriteKeyValue Variables GameDrawer.SortMode Alphabetically "#@#Settings.inc"]')
	end
	Filter()
	Draw()
	ScrollToFront()
	...
	for i = 0,(math.min((NUMBANNERS+1),NUMGAMES)) do
		...
		if index ~= 0 and index <= NUMGAMES then
			...
			if listNoBanner[listView[index]['bannerName']] ~= nil then
				SKIN:Bang('[!SetOption Game'..i..' MouseOverAction """[!CommandMeasure TileAnimation "Execute '..soundindex..'"]"""] [!SetOption Game'..i..' MouseLeaveAction """[!CommandMeasure TileAnimation "Execute '..(soundindex+1)..'"]"""] [!SetOption Game'..i..'T Text "No banner...\n'..listView[index]['appName']..'\nLast Played: '..time..'"] [!SetOption Game'..i..'T LeftMouseUpAction """[!CommandMeasure GameDrawer """Launch('..index..')"""]"""] [!SetOption Game'..i..'T Fontcolor #Color.Main#,(255-(((Clamp((([*SlideProgress*]*[*SizeCalc*])+20),20,(#*Height*#-42))-20)/(#*Height*#-62))*255))] [!SetOption Game'..i..'C Fontcolor #Color.Main#,(255-(((Clamp((([*SlideProgress*]*[*SizeCalc*])+20),20,(#*Height*#-42))-20)/(#*Height*#-62))*255))] [!SetOption Game'..i..'C LeftMouseUpAction """[!CommandMeasure GameDrawer """Hide("'..listView[index]['appID']..'",'..index..')"""]"""] [!SetOption Game'..i..'B ImageAlpha (255-(((Clamp((([*SlideProgress*]*[*SizeCalc*])+20),20,(#*Height*#-42))-20)/(#*Height*#-62))*255))]')
			else
				SKIN:Bang('[!SetOption Game'..i..' MouseOverAction """[!CommandMeasure TileAnimation "Execute '..soundindex..'"]"""] [!SetOption Game'..i..' MouseLeaveAction """[!CommandMeasure TileAnimation "Execute '..(soundindex+1)..'"]"""] [!SetOption Game'..i..' ImageName "#SKINSPATH#NXT-OS Data\\GameDrawer\\Banners\\'..listView[index]['bannerName']..'"] [!SetOption Game'..i..' LeftMouseUpAction """[!CommandMeasure GameDrawer """Launch('..index..')"""]"""] [!SetOption Game'..i..' RightMouseUpAction """[!CommandMeasure GameDrawer """ToggleHideMenu('..i..')"""]"""] [!SetOption Game'..i..' ImageAlpha (255-(((Clamp((([*SlideProgress*]*[*SizeCalc*])+20),20,(#*Height*#-42))-20)/(#*Height*#-62))*255))] [!SetOption Game'..i..'T Text "'..listView[index]['appName']..'\nLast Played: '..time..'"] [!SetOption Game'..i..'T LeftMouseUpAction """[!CommandMeasure GameDrawer """Launch('..index..')"""]"""] [!SetOption Game'..i..'T Fontcolor #Color.Main#,0] [!SetOption Game'..i..'C Fontcolor #Color.Main#,0] [!SetOption Game'..i..'C LeftMouseUpAction """[!CommandMeasure GameDrawer """Hide("'..listView[index]['appID']..'",'..index..')"""]"""] [!SetOption Game'..i..'B ImageAlpha 0]')
			end
		end
	end
end